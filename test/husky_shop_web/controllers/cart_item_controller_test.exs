defmodule HuskyShopWeb.CartItemControllerTest do
  ...
  def fixture(:cart_item) do
    {:ok, cart_item} = Carts.create_cart_item(@create_attrs)
    cart_item
  end
  ...
  describe "new cart_item" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.cart_item_path(conn, :new))
  ...
  describe "create cart_item" do
    test "redirects to show when data is valid", %{conn: conn} do
      ...
      assert redirected_to(conn) == Routes.cart_item_path(conn, :show, id)
  ...
  describe "edit cart_item" do
    ...
    test "renders form for editing chosen cart_item", %{conn: conn, cart_item: cart_item} do
      conn = get(conn, Routes.cart_item_path(conn, :edit, cart_item))
  ...
  describe "update cart_item" do
    ...
    test "redirects when data is valid", %{conn: conn, cart_item: cart_item} do
      ...
      assert redirected_to(conn) == Routes.cart_item_path(conn, :show, cart_item)

      conn = get(conn, Routes.cart_item_path(conn, :show, cart_item))
  ...
  describe "delete cart_item" do
    ...
    test "deletes chosen cart_item", %{conn: conn, cart_item: cart_item} do
      ...
      assert redirected_to(conn) == Routes.cart_item_path(conn, :index)
